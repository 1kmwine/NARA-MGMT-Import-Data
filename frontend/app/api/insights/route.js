...
export async function POST(req) {
  ...
  const prompt = [
    ...
    '아래 통계 JSON을 바탕으로, 임원 보고용 한 줄 인사이트 2개를 한국어로 작성해라.',
    '임원은 이 두 줄만 읽고 다음 액션을 판단한다 — 숫자 요약이 아니라 "왜 중요한지·',
    '무엇을 봐야 하는지"가 드러나야 한다. stats에 성장/감소 상위 리스트나 세부구성이',
    '있으면 그 중 가장 의미 있는 한두 개를 골라 원인·맥락을 짚어라(예: 특정 항목이',
    '전체 증감을 주도했는지, 상위국 중 이례적으로 급등/급락한 곳이 있는지).',
    '',
    '규칙:',
    '- 각 문장은 반드시 주어진 숫자에 근거해야 하고, 숫자를 문장에 포함해라.',
    '- 한 문장 60자 내외, 반말/구어체 금지, 보고서체("~함", "~주도", "~유지" 등).',
    '- 숫자를 지어내지 말고 stats에 있는 값만 사용해라.',
    '- "증가함"/"감소함"처럼 방향만 말하는 문장 금지 — 규모(YoY_Gap/GRW)와 비교 대상',
    '  (전체 대비, 다른 항목/국가 대비)까지 한 문장 안에 엮어라.',
    stats.포커스 ? '- stats.포커스가 있으면 그 범위(포커스.범위)에 대해서만 작성해라 — 다른 국가/항목 언급 금지.' : '',
